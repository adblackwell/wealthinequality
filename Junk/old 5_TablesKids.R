...
#### Tables ####
################
load("alldataWKids.Rdata")
## Table 1: descriptives
alldata$depends<-rowSums(!is.na(alldata[,c("BMI.TZ","SumDiagZ","InfectiousDiag","RepiratoryDiag","GIDiag")]))
...
ft<-t(apply(alldata2[,c("BMI","sum.diag","InfectiousDiag","RepiratoryDiag","GIDiag","Age","male","AgeCorWealth","ComunSize", "route.distance.SB","MeanWealth.AgeCor","Gini.AgeCor")],2,desc_stats))
colnames(ft)<-c("N","Median","SD","Low","High")
dummy<-rep(NA,5)
ft<-rbind(dummy,ft[1:5,],dummy,ft[6:12,])
ft<-data.frame(ft)
ft$Variable<-c("Outcomes: Health", "BMI [kg/m2] a","Total Morbidity [count] b","Infections/parasites [yes/no] b","Respiratory disease [yes/no] b","Gastrointestinal [yes/no] b","Predictors","Age [years]","Sex [0=female, 1=male]","Household wealth [Bs]","Community Size [Adults >15]","Distance to market town [km]","Mean Community Wealth [Bs]","Community Wealth inequality [Gini]")
...
ft<-autofit(ft)
ft<-add_footer_lines(ft,values=c("a Whether higher or lower BMI is better is a bit ambiguous: in high-income countries higher BMI is associated with worse health, lower status and greater inequality, whereas in low-income countries the reverse may be true.","b see Supplementary Table S1 for an overview of the most common morbidities by category."))
