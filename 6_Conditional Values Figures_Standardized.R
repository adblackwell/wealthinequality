...
### relative wealth
tiff("All wealth effects no interactions.ADB.R2.Standardized.tiff", compression="lzw", height=9, width=10, units="cm", res=600, pointsize=5)
...
### inequality
  ...
  tiff("All gini effects no interactions.ADB.R2.Standardized.tiff", compression="lzw", height=9, width=10, units="cm", res=600, pointsize=5)
  par(mfrow=c(3,5), oma=c(3,3,0,0), mar=c(2,2,3,1))
...
### absolute wealth
{
  ## have to calculate predictions "by hand", since absolute wealth effect not in marginal_effects
  ## since there are different size effects for village and individual wealth, need to do all realistic combinations
  absHHWealth<-seq(-3, 3, length=100)
  absHHWealth2<- expand.grid(HHWealthZ = absHHWealth, MeanWealthZ = seq(-1,1,length=50))
  absHHWealth2$HHWealthZ<-absHHWealth2$HHWealthZ-absHHWealth2$MeanWealthZ
  
  getCond<-function(mod){
    post<-posterior_samples(mod) 
    means<-colMeans(mod$data[,1:8])
    mu.link<- function(x) post$b_Intercept+post$b_HHWealthZ*x[3]+post$b_MeanWealthZ*x[2]+post$b_male*means["male"] + post$b_GiniZ*means["GiniZ"]+post$b_AgeZ*means["AgeZ"]+post$b_DistZ*means["DistZ"]+post$b_SizeZ*means["SizeZ"]
    mu<- apply(absHHWealth2,1,mu.link)
    mu.mean<-sapply(absHHWealth,function(x) mean(mu[,which(absHHWealth2$HHWealthZ==x)]))
    mu.HPDI<-t(sapply(absHHWealth,function(x) HPDI(as.vector(mu[,which(absHHWealth2$HHWealthZ==x)]), prob =0.95)))
    mu.return<-cbind(mu.mean,mu.HPDI)
  }
  
  dep.mu<-getCond(dep)
  conf.mu<-getCond(conf)
  othp.mu<-getCond(othp)
  lab.mu<-getCond(lab)
  cort.mu<-getCond(cort)
  bmi.mu<-getCond(bmi)
  sys.mu<-getCond(sys)
  dias.mu<-getCond(dias)
  salud.mu<-getCond(salud)
  sumdiag.mu<-getCond(sumdiag)
  infect.mu<-getCond(infect)
  resp.mu<-getCond(resp)
  gastro.mu<-getCond(gastro)
  
  
  tiff("All absolute wealth.ADB.R2.tiff", compression="lzw", height=9, width=10, units="cm", res=600, pointsize=5)
  par(mfrow=c(3,5), oma=c(3,3,0,0), mar=c(2,2,3,1))
  plot(dep.mu[,1]~absHHWealth, type="l", ylim=c(-0.75,0.75), xlim=c(-3,3), main="Depression", col=wes_palettes$Royal2[1], lwd=2, ylab="Predicted outcome [Z]", xlab="")
  polygon(c(absHHWealth, rev(absHHWealth)), c(smooth(dep.mu[,2]), rev(smooth(dep.mu[,3]))), col=adjustcolor(wes_palettes$Royal2[1], alpha.f=0.5), border=NA)
  text(0,0.7,labels=format(round(1-dep.PP[length(dep.PP)],digits=2), nsmall=2), cex=1.5)
  mtext(text="Predicted outcome [Z]",side=2,line=2.5)
  
  plot(conf.mu[,1]~absHHWealth, type="l", ylim=c(-0.75,0.75), xlim=c(-3,3), main="Conflicts", col=wes_palettes$Royal2[1], lwd=2, ylab="", xlab="")
  polygon(c(absHHWealth, rev(absHHWealth)), c(smooth(conf.mu[,2]), rev(smooth(conf.mu[,3]))), col=adjustcolor(wes_palettes$Royal2[1], alpha.f=0.5), border=NA)
  text(0,0.7,labels=format(round(1-conf.PP[length(conf.PP)],digits=2), nsmall=2), cex=1.5)
  
  plot(lab.mu[,1]~absHHWealth, type="l", ylim=c(-0.75,0.75), xlim=c(-3,3), main="Fewer Labor Partners", col=wes_palettes$Royal2[1], lwd=2, ylab="", xlab="")
  polygon(c(absHHWealth, rev(absHHWealth)), c(smooth(lab.mu[,2]), rev(smooth(lab.mu[,3]))), col=adjustcolor(wes_palettes$Royal2[1], alpha.f=0.5), border=NA)
  text(0,0.7,labels=format(round(1-lab.PP[length(lab.PP)],digits=2), nsmall=2), cex=1.5)
  
  plot(othp.mu[,1]~absHHWealth, type="l", ylim=c(-0.75,0.75), xlim=c(-3,3), main="Non-social Problems", col=wes_palettes$Royal2[1], lwd=2, ylab="", xlab="")
  polygon(c(absHHWealth, rev(absHHWealth)), c(smooth(othp.mu[,2]), rev(smooth(othp.mu[,3]))), col=adjustcolor(wes_palettes$Royal2[1], alpha.f=0.5), border=NA)
  text(0,0.7,labels=format(round(1-othp.PP[length(othp.PP)],digits=2), nsmall=2), cex=1.5)
  
  plot(cort.mu[,1]~absHHWealth, type="l", ylim=c(-0.75,0.75), xlim=c(-3,3), main="Cortisol", col=wes_palettes$Royal2[1], lwd=2, ylab="", xlab="")
  polygon(c(absHHWealth, rev(absHHWealth)), c(smooth(cort.mu[,2]), rev(smooth(cort.mu[,3]))), col=adjustcolor(wes_palettes$Royal2[1], alpha.f=0.5), border=NA)
  text(0,0.7,labels=format(round(1-cort.PP[length(cort.PP)],digits=2), nsmall=2), cex=1.5)
  
  plot(bmi.mu[,1]~absHHWealth, type="l", ylim=c(-0.75,0.75), xlim=c(-3,3), main="Body Mass Index", col=wes_palettes$Royal2[3], lwd=2, ylab="Predicted outcome [Z]", xlab="")
  polygon(c(absHHWealth, rev(absHHWealth)), c(smooth(bmi.mu[,2]), rev(smooth(bmi.mu[,3]))), col=adjustcolor(wes_palettes$Royal2[3], alpha.f=0.5), border=NA)
  text(0,0.7,labels=format(round(1-bmi.PP[length(bmi.PP)],digits=2), nsmall=2), cex=1.5)
  mtext(text="Predicted outcome [Z]",side=2,line=2.5)
  
  plot(sys.mu[,1]~absHHWealth, type="l", ylim=c(-0.75,0.75), xlim=c(-3,3), main="Systolic BP", col=wes_palettes$Royal2[3], lwd=2, ylab="", xlab="")
  polygon(c(absHHWealth, rev(absHHWealth)), c(smooth(sys.mu[,2]), rev(smooth(sys.mu[,3]))), col=adjustcolor(wes_palettes$Royal2[3], alpha.f=0.5), border=NA)
  text(0,0.7,labels=format(round(1-sys.PP[length(sys.PP)],digits=2), nsmall=2), cex=1.5)
  
  plot(dias.mu[,1]~absHHWealth, type="l", ylim=c(-0.75,0.75), xlim=c(-3,3), main="Diastolic BP", col=wes_palettes$Royal2[3], lwd=2, ylab="", xlab="")
  polygon(c(absHHWealth, rev(absHHWealth)), c(smooth(dias.mu[,2]), rev(smooth(dias.mu[,3]))), col=adjustcolor(wes_palettes$Royal2[3], alpha.f=0.5), border=NA)
  text(0,0.7,labels=format(round(1-dias.PP[length(dias.PP)],digits=2), nsmall=2), cex=1.5)
  
  plot(salud.mu[,1]~absHHWealth, type="l", ylim=c(-0.75,0.75), xlim=c(-3,3), main="Worse Self-rated Health", col=wes_palettes$Royal2[3], lwd=2, ylab="", xlab="")
  polygon(c(absHHWealth, rev(absHHWealth)), c(smooth(salud.mu[,2]), rev(smooth(salud.mu[,3]))), col=adjustcolor(wes_palettes$Royal2[3], alpha.f=0.5), border=NA)
  text(0,0.7,labels=format(round(1-salud.PP[length(salud.PP)],digits=2), nsmall=2), cex=1.5)
  
  plot(sumdiag.mu[,1]~absHHWealth, type="l", ylim=c(-0.75,0.75), xlim=c(-3,3), main="Total Morbidity", col=wes_palettes$Royal2[3], lwd=2, ylab="", xlab="")
  polygon(c(absHHWealth, rev(absHHWealth)), c(smooth(sumdiag.mu[,2]), rev(smooth(sumdiag.mu[,3]))), col=adjustcolor(wes_palettes$Royal2[3], alpha.f=0.5), border=NA)
  text(0,0.7,labels=format(round(1-sumdiag.PP[length(sumdiag.PP)],digits=2), nsmall=2), cex=1.5)
  
  plot(logistic(infect.mu[,1])~absHHWealth, type="l", ylim=c(0,1), xlim=c(-3,3), main="Infections", col=wes_palettes$Royal2[5], lwd=2, ylab="Predicted outcome [Z]", xlab="Absolute HH Wealth [Z]")
  polygon(c(absHHWealth, rev(absHHWealth)), c(smooth(logistic(infect.mu[,2])), rev(smooth(logistic(infect.mu[,3])))), col=adjustcolor(wes_palettes$Royal2[5], alpha.f=0.5), border=NA)
  text(0,0.9,labels=format(round(1-infect.PP[length(infect.PP)],digits=2), nsmall=2), cex=1.5)
  mtext("Predicted Probability",side=2,line=2.5)
  
  plot(logistic(resp.mu[,1])~absHHWealth, type="l", ylim=c(0,1), xlim=c(-3,3), main="Respiratory", col=wes_palettes$Royal2[5], lwd=2, ylab="", xlab="Absolute HH Wealth [Z]")
  polygon(c(absHHWealth, rev(absHHWealth)), c(smooth(logistic(resp.mu[,2])), rev(smooth(logistic(resp.mu[,3])))), col=adjustcolor(wes_palettes$Royal2[5], alpha.f=0.5), border=NA)
  text(0,0.9,labels=format(round(1-resp.PP[length(resp.PP)],digits=2), nsmall=2), cex=1.5)
  
  plot(logistic(gastro.mu[,1])~absHHWealth, type="l", ylim=c(0,1), xlim=c(-3,3), main="Gastrointestinal", col=wes_palettes$Royal2[5], lwd=2, ylab="", xlab="Absolute HH Wealth [Z]")
  polygon(c(absHHWealth, rev(absHHWealth)), c(smooth(logistic(gastro.mu[,2])), rev(smooth(logistic(gastro.mu[,3])))), col=adjustcolor(wes_palettes$Royal2[5], alpha.f=0.5), border=NA)
  text(0,0.9,labels=format(round(1-gastro.PP[length(gastro.PP)],digits=2), nsmall=2), cex=1.5)

  mtext(text="Absolute Household Wealth [Z]",side=1,line=1,outer=TRUE)
  dev.off()
